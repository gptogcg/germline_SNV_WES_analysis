...
germinals <- which(SAMPLES[,"MOSTRA"]=="germinal" & SAMPLES[,"ESTUDI"]=="CCR")
...
inner_frequency_filter <- which(EXOMES_1.0.2$inner_frequency <= 0.250) ## no filtra sino que agarra todas ## ##Acá hace falta agregar los NAs##
...
df_CLL_2.0 <- read.table("ANTELO_01__df_CLL_2.0.txt", header = T, sep = "\t")##---agregado por Mariano 29/10/2017---##
...
fq_filters <- which( (EXOMES_5.0$GMAF <= 0.01 | is.na(EXOMES_5.0$GMAF)) 
                   ...
                   & (EXOMES_5.0$ESP6500_EA_AF <= 0.01 | is.na(EXOMES_5.0$ESP6500_EA_AF)) 
                   & (EXOMES_5.0$ExAC_AF <= 0.01 | is.na(EXOMES_5.0$ExAC_AF)) 
                   & (EXOMES_5.0$inner_frequency <=0.25))
